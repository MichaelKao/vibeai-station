...
export async function save(file){
  ...
  const name = path.basename(key); fs.writeFileSync(path.join('data/uploads',name),file.buffer); return '/uploads/'+name;
}
export async function remove(url){
  ...
  else if (url.startsWith('/uploads/')) fs.unlinkSync(path.join('data',url)); }catch{}
}
