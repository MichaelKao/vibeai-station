...
import { UPLOAD_DIR } from '../src/paths.js';
...
for (const [name, nick, intro, admin] of USERS) {
  const ex = one('SELECT id FROM users WHERE name=?', name);
  if (ex) { uid[name] = ex.id; continue; }
  const s = salt();
  const r = run(`INSERT INTO users(name,pass,salt,nick,intro,admin,visits,today_hits,hits_date,
...
async function photo(keyword, tint) {
  ...
  if (fs.existsSync(cache)) {
    fs.copyFileSync(cache, dest); cached++;
    return '/uploads/' + name;
  }
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      await sharp(buf).jpeg({ quality: 82 }).toFile(cache);
      fs.copyFileSync(cache, dest); fetched++;
      return '/uploads/' + name;
    } catch {
      if (attempt < 3) await sleep(1200 * attempt);
    }
  }
  // 抓不到就從已經抓到的真實照片裡循環取一張。
  // 純色方塊會讓整個版面看起來很假，也試不出真實照片的明暗與比例；
  // 重複用真照片遠比塞色塊好。真的一張都沒有（完全離線）才退回純色。
  failed++;
  const pool = fs.readdirSync(CACHE).filter(f => f.endsWith('.jpg'));
  if (pool.length) {
    fs.copyFileSync(path.join(CACHE, pool[made % pool.length]), dest);
    return '/uploads/' + name;
  }
  const [r2, g2, b2] = [1, 3, 5].map(i => parseInt(tint.slice(i, i + 2), 16));
  await sharp({ create: { width: 640, height: 480, channels: 3, background: { r: r2, g: g2, b: b2 } } })
    .jpeg({ quality: 80 }).toFile(dest);
  return '/uploads/' + name;
}

const TINTS = ['#E48A41', '#447AC4', '#A1D344', '#CB3939', '#8AB9F4', '#FFD196', '#72A316', '#C94F7C'];
// 每本相簿配一組搜尋關鍵字，照片內容才會跟標題對得上
const ALBUMS = [
  ['宜蘭兩天一夜', '跟朋友衝一波，民宿超讚', '國內旅遊', '台灣', 'taiwan,countryside'],
  ['東京自由行', '第一次一個人出國', '國外旅遊', '世界各地', 'tokyo,street'],
  ['我家的貓', '牠又把衛生紙咬爛了', '心肝寵物', '台灣', 'cat'],
  ['夜市美食紀錄', '一週吃了五次', '美食記錄', '台灣', 'streetfood,night'],
  ['畢業typo紀念', '再也回不去了', '學園生活', '台灣', 'graduation'],
  ['隨手拍', '手機拍的', '專業攝影', '台灣', 'city,street'],
];
...
for (const [name] of USERS) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (one('SELECT 1 FROM albums WHERE user_id=? AND title=?', uid[name], title)) continue;
    const a = run(`INSERT INTO albums(user_id,title,descr,topic,place,views,featured)
    ...
    for (let k = 0; k < 5 + (i % 4); k++) {
      const url = await photo(keyword, pick(TINTS, uid[name] + k));
      const st = fs.statSync(path.join(UPLOAD_DIR, path.basename(url)));
      run('INSERT INTO photos(album_id,url,thumb,caption,bytes,views) VALUES(?,?,?,?,?,?)',
        aid, url, url, pick(['這天天氣超好', '好吃！', '牠在生氣', '順便自拍一張', ''], k),
        st.size, Math.floor(Math.random() * 300));
      if (!cover) cover = url;
    }
    run('UPDATE albums SET cover=? WHERE id=?', cover, aid);
  }
}
...
// 預設那張卡通圖示放在「名家專欄」「投稿精選」這種以人為主的模組裡特別假。
{
  ...
  for (const [name] of USERS) {
    ...
    const out = `avatar_${name}.jpg`;
    await sharp(src).resize(90, 90, { fit: 'cover', position: 'centre' })
      .jpeg({ quality: 85 }).toFile(path.join(UPLOAD_DIR, out));
    run('UPDATE users SET avatar=? WHERE id=?', '/uploads/' + out, uid[name]);
    n++;
  }
  console.log(`大頭貼 ${n} 張（90x90，裁自真實照片）`);
}

console.log(`相簿 ${one('SELECT count(*) c FROM albums').c} 本 / 照片 ${one('SELECT count(*) c FROM photos').c} 張`
...
for (const [name] of USERS) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (one('SELECT 1 FROM posts WHERE user_id=? AND title=?', uid[name], title)) continue;
    const r = run(`INSERT INTO posts(user_id,title,body,category,topic,mood,weather,views,likes,featured)
    ...
    for (let c = 0; c < (i % 4); c++)
      run('INSERT INTO comments(post_id,author,body,reply) VALUES(?,?,?,?)', pid,
        pick(['小明', '阿華', '路人甲', 'Tina'], c),
        pick(['搶頭香！', '坐沙發～', '推推推', '我也想去！'], c),
        c === 0 ? '謝謝你的留言 :)' : '');
  }
}
console.log(`網誌 ${one('SELECT count(*) c FROM posts').c} 篇 / 迴響 ${one('SELECT count(*) c FROM comments').c} 則`);

// ---- 留言板 / 好友 / 訪客 / 公告 ----
for (const [name] of USERS) {
  for (let i = 0; i < 4; i++)
    if (!one('SELECT 1 FROM guestbook WHERE user_id=? AND author=?', uid[name], pick(USERS, i + 1)[1]))
      run('INSERT INTO guestbook(user_id,author,subject,body,secret,reply) VALUES(?,?,?,?,?,?)',
  ...
  for (const [other] of USERS)
    if (other !== name && Math.random() < 0.45)
      run('INSERT OR IGNORE INTO friends(user_id,friend_id,grp) VALUES(?,?,?)',
        uid[name], uid[other], pick(['好友', '同學', '同事', '網友'], other.length));
  for (let i = 0; i < 5; i++)
    run('INSERT INTO visitors(user_id,who) VALUES(?,?)', uid[name], pick(USERS, i + 2)[0]);
  const p = one('SELECT id,title FROM posts WHERE user_id=? ORDER BY id DESC', uid[name]);
  if (p) run('INSERT INTO acts(user_id,kind,title,url) VALUES(?,?,?,?)',
    uid[name], 'blog', p.title, `/${name}/blog/${p.id}`);
}

if (!one('SELECT 1 FROM notices'))
  for (const b of ['小站開張囉！完全免費，歡迎申請帳號～',
    '[公告] 相簿空間每人 500MB，請大家珍惜使用。',
    '[活動] 第一屆站慶攝影比賽開始徵件！'])
    run('INSERT INTO notices(body) VALUES(?)', b);

console.log(`留言 ${one('SELECT count(*) c FROM guestbook').c} 則 / 好友 ${one('SELECT count(*) c FROM friends').c} 組`);
